...
\begin{figure}[ht]
    \centering
    \includestandalone[width=0.75\textwidth]{figures/fig_06_pregelArchitecture}
...
As a brief introduction to this section, let's first define what validation -- in the context of Knowledge graphs -- is. Wikibase manages its data following the FAIR principles: \fb{findable}, \fb{accessible}, \fb{interoperable} and \fb{reusable}~\cite{10.1038/sdata.2016.18}. Thus, the distributed nature of Wikibase affects the \textit{validity} of the data it contains. We must distinguish a \textit{well-formed} document: syntactically correct, from a \textit{valid} one, where in addition to being \textit{well-formed}, it must conform to some type definition. We have seen in section \ref{section:wikibase:serialization}, the different serialization formats supported by Wikidata, being RDF one of those.