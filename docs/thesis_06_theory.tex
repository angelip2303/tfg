...
\inputminted[highlightlines={3,6,7,9},highlightcolor=blueUniovi!15]{turtle}{code/listings/3-2_serialization.rdf}
...
\begin{itemize}
    \item \mintinline[fontsize=\small]{scala}{mapVertices(g: Graph[|\VertSet|,|\EdgeSet|], f: (Id,|\VertSet|)|$\rightarrow$||\VertSet|)): Graph[|\VertSet|,|\EdgeSet|]}
          \begin{description}
              \item[Description] maps every pair \texttt{(id, v)} in the vertices of \texttt{g} to \texttt{id, f(v)}
          \end{description}
    \item \mintinline[fontsize=\small]{scala}{mapReduceTriples(g: Graph[|\VertSet|,|\EdgeSet|], m: (|\VertSet|,|\EdgeSet|,|\VertSet|)|$\rightarrow$|(Id,|\MsgSet|)), r: (|\MsgSet|,|\MsgSet|)|$\rightarrow$||\MsgSet|)): RDD[Id,|\MsgSet|]}
