...
Lastly, an statement is a piece of data about an item, and is recorded in the page of the item itself. Putting it all together, the way Wikibase manages information is as follows: through an statement, we add information to a certain item using some property and its value.
...
The main purpose of Wikidata is to store data about things that are described by pages in Wikipedia (in any language)\footnote{\url{https://www.mediawiki.org/wiki/Wikibase/DataModel}}.

Putting it all together, let's formally define what a Wikibase graph is.
...
was designed with the idea in mind of representing all the structures of the Wikibase data model in RDF. As an example to that, a fragment of the information about Alan Turing that models one of his employers is represented as follows\footnote{\url{https://www.wikidata.org/wiki/Special:EntityData/Q7251.ttl}}:

\begin{example}
    \alanTuring was \employer by \government between 1938 and 1945.
\end{example}
