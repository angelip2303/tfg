...
This way, what we are achieving is a database where we link concepts in a language-agnostic approach. Notice that in the previous example there is only one \textit{statement} which relates the \textit{item} \texttt{Q11678} with the \textit{value} \texttt{Q15062348} through the \textit{property} \texttt{P136}. We are linking concepts. In case we want to provide a translation to some of those entities, you may -- for example -- add information about the items or provide the appropriate name in a certain language. What will remain unchanged is the relation previously described.
...
\begin{table}[h]
    \begin{tabular}{ccl}
        ...
        $\StmtSet$    & = \{ & (\alanTuring, \instanceOf, \Human, \{\}),                                                        \\
                      ...
                      &      & (\alanTuring, \dateOfDeath, {\small 7 June 1954}4, \{\}),                                        \\
